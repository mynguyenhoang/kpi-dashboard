...
# Bỏ ép size chữ khổng lồ đi, trả về layout tiêu chuẩn
def clean_layout(fig, title):
    ...
    # Tắt tính năng tự cắt mép biểu đồ để số được hiển thị thoải mái
...
def render_dashboard(df, summary, primary_color):
    ...
    with col1:
        fig_vol = go.Figure()
        # Đưa về size 14 vừa vặn cho line chart
    ...
    with col2:
        ...
        # Đưa về size 13 cho bar chart để các cột không bị cấn nhau
    ...
    with col_l1:
        ...
        fig_sh_late.add_trace(go.Bar(
            ...
            textposition='outside', textfont=dict(size=14, color='#b91c1c')
        ))
        fig_sh_late = clean_layout(fig_sh_late, "Shuttle Xuất Phát Trễ")
        max_y_sh = df['Shuttle Late'].max() * 1.15 if df['Shuttle Late'].max() > 0 else 5
    ...
    with col_l2:
        ...
        fig_lh_late.add_trace(go.Bar(
            ...
            textposition='outside', textfont=dict(size=14, color='#be123c')
        ))
        fig_lh_late = clean_layout(fig_lh_late, "Linehaul Xuất Phát Trễ")
        max_y_lh = df['Linehaul Late'].max() * 1.15 if df['Linehaul Late'].max() > 0 else 5
