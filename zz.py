...
def get_data():
    ...
    # --- ĐÂY LÀ PHẦN SỬA LOGIC MTD ---
    # Tìm ngày cuối cùng thực tế có dữ liệu (dựa trên hàng Inbound Vol của HCM - hàng 4)
    actual_days = 0
    test_start = start_col_idx if start_col_idx != -1 else 6
    for i in range(31):
        if pd.notna(clean_val(4, test_start + i)):
            actual_days = i + 1
        else:
            break
    ...
    cols_to_scan = [test_start + i for i in range(num_days)]
...
def render_dashboard(df, summary, primary_color):
    if df.empty: return
    # TÍNH TOÀN MTD TRÊN TẬP DỮ LIỆU ĐÃ LỌC NGÀY THỰC TẾ
    ...
    t_bl = df['Backlog'].iloc[-1] if not df['Backlog'].empty else 0 # Backlog lấy ngày cuối cùng thực tế
    ...
    c4.metric("Backlog (Hiện tại) | 积压货物", format_vietnam(t_bl))
    st.markdown("<br>", unsafe_allow_html=True)

    # 2. BẢNG TỔNG HỢP SONG NGỮ (GIỮ NGUYÊN GỐC)
    html_table = f"""
    ...
    with col_chart1:
        ...
        fig_vol.update_layout(title="Sản lượng Inbound & Outbound hàng ngày", plot_bgcolor='white', margin=dict(t=40, l=10, r=10, b=10))
    ...
    with col_chart2:
        ...
        fig_ms.add_trace(go.Scatter(x=df['Ngày'], y=df['Tỷ lệ Missort (%)'], name="Tỷ lệ %", mode='lines+markers', line=dict(color='#ef4444')), secondary_y=True)
        fig_ms.update_layout(title_text="Phân tích Missort", plot_bgcolor='white', margin=dict(t=40, l=10, r=10, b=10))
        st.plotly_chart(fig_ms, use_container_width=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"<h4 style='color: {primary_color}; font-size: 18px;'>2. Quản lý Vận Tải & Hàng Tồn</h4>", unsafe_allow_html=True)
    col_chart3, col_chart4 = st.columns(2)
    with col_chart3:
        fig_xe = go.Figure()
        fig_xe.add_trace(go.Bar(x=df['Ngày'], y=df['LH Đúng Giờ'].fillna(0)+df['Shuttle Đúng Giờ'].fillna(0), name="Đúng giờ", marker_color='#10b981'))
        fig_xe.add_trace(go.Bar(x=df['Ngày'], y=df['LH Trễ'].fillna(0)+df['Shuttle Trễ'].fillna(0), name="Trễ giờ", marker_color='#f43f5e'))
        fig_xe.update_layout(title="Kiểm soát Chuyến xe chạy COT", barmode='stack', plot_bgcolor='white', margin=dict(t=40, l=10, r=10, b=10))
