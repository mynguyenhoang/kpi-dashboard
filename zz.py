...
def get_data():
    ...
    # Xác định các cột dữ liệu theo ngày
    date_row_idx = 3
    start_col_idx = -1
    ...
    num_days = max_day if start_col_idx != -1 else 26
...
def add_stacked_bar_with_total(fig, df, on_time_col, late_col, title):
    # Tính tổng cho label
    temp_df = df.copy()
    temp_df[on_time_col] = temp_df[on_time_col].fillna(0)
    temp_df[late_col] = temp_df[late_col].fillna(0)
    temp_df['Total'] = temp_df[on_time_col] + temp_df[late_col]

    # Trace Đúng (Xanh)
    fig.add_trace(go.Bar(
        x=df['Ngày'], y=df[on_time_col], name="Đúng",
        marker_color='#10b981', text=df[on_time_col].apply(lambda x: int(x) if x > 0 else ""),
        textposition='inside'
    ))
    # Trace Trễ (Đỏ)
    fig.add_trace(go.Bar(
        x=df['Ngày'], y=df[late_col], name="Trễ",
        marker_color='#f43f5e', text=df[late_col].apply(lambda x: int(x) if x > 0 else ""),
        textposition='inside'
    ))
    # Trace tàng hình để hiện TỔNG ở trên đầu
    fig.add_trace(go.Bar(
        x=temp_df['Ngày'], y=temp_df['Total'], name="Tổng",
        marker_color='rgba(0,0,0,0)', hoverinfo='none', showlegend=False,
        text=temp_df['Total'].apply(lambda x: int(x) if x > 0 else ""),
        textposition='outside'
    ))
    fig.update_layout(title=title, barmode='stack', plot_bgcolor='white', 
...
def render_dashboard(df, summary, primary_color):
    ...
    with col1:
        ...
        fig_vol.update_layout(title="Inbound & Outbound", plot_bgcolor='white', margin=dict(t=40, b=10), legend=dict(orientation="h", y=1.1))
        st.plotly_chart(fig_vol, use_container_width=True)
    with col2:
        fig_p1 = px.bar(df, x="Ngày", y="Total Process Vol", title="Năng suất (Đơn)", text_auto='.2s')
        fig_p1.update_traces(marker_color='#38bdf8').update_layout(plot_bgcolor='white')
        st.plotly_chart(fig_p1, use_container_width=True)
    with col3:
        fig_p2 = px.bar(df, x="Ngày", y="Total Process Wgt", title="Năng suất (Kg)", text_auto='.2s')
        fig_p2.update_traces(marker_color='#818cf8').update_layout(plot_bgcolor='white')
        st.plotly_chart(fig_p2, use_container_width=True)

    # Section 4: Vận tải & Hàng tồn (ĐÃ CẬP NHẬT TỔNG XE)
    st.markdown(f"<h4 style='color: {primary_color};'>2. Quản lý Vận Tải & Hàng Tồn</h4>", unsafe_allow_html=True)
    col4, col5, col6 = st.columns(3)
    with col4:
        fig_lh = go.Figure()
        fig_lh = add_stacked_bar_with_total(fig_lh, df, 'LH Đúng Giờ', 'LH Trễ', "Linehaul (LH)")
        st.plotly_chart(fig_lh, use_container_width=True)
    with col5:
        fig_sh = go.Figure()
        fig_sh = add_stacked_bar_with_total(fig_sh, df, 'Shuttle Đúng Giờ', 'Shuttle Trễ', "Shuttle (ST)")
...
if data_hcm[0].empty and data_bn[0].empty:
    st.info("Đang chờ dữ liệu từ Feishu...")
else:
