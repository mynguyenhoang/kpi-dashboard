...
# ==============================================================================
# 2. HÀM LẤY DỮ LIỆU TỪ FEISHU (CHỈ CẬP NHẬT ĐỂ LƯU THÊM TỔNG CHUYẾN XE)
# ==============================================================================
def get_tenant_access_token():
    try:
        url = "https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal"
        # Lưu ý bảo mật: app_id/app_secret nên dùng secrets management
        payload = {
...
def get_data():
    ...
    def extract_hub_data(vin_idx, vout_idx, win_idx, wout_idx, tproc_vol_idx, tproc_wgt_idx, ms_idx, ms_rt_idx, bl_idx, lhc_idx, lht_idx, shc_idx, sht_idx):
        ...
        # --- LOGIC GIỮ NGUYÊN, THÊM CỘT TỔNG ĐỂ HIỂN THỊ TRÊN BIỂU ĐỒ ---
        # Logic tính "Đúng Giờ" và "Trễ" dựa trên tổng và trễ giữ nguyên.
        ...
        data["LH Tổng"] = [c if c > 0 else np.nan for c in lh_c_list] # Dùng để hiển thị nhãn
        ...
        data["Shuttle Tổng"] = [c if c > 0 else np.nan for c in sh_c_list] # Dùng để hiển thị nhãn
        # -------------------------------------------------------------
    ...
    # Lấy dữ liệu HCM và BN với các index dòng tương ứng (GIỮ NGUYÊN)
...
# ==============================================================================
# 3. GIAO DIỆN (CHỈ CẬP NHẬT PHẦN VẼ BIỂU ĐỒ)
# ==============================================================================
...
# Hàm tính toán cell WOW (Giữ nguyên)
...
# ==============================================================================
# HÀM CHÍNH ĐỂ RENDER DASHBOARD (CHỨA SỰ THAY ĐỔI BIỂU ĐỒ)
# ==============================================================================
def render_dashboard(df, summary, primary_color):
    if df.empty: return

    # Tính toán tổng MTD (Giữ nguyên)
    ...
    # Tính toán tỷ lệ đúng giờ MTD dựa trên logic cũ (Tổng - Trễ) - GIỮ NGUYÊN LOGIC
    lh_total_mtd = df['LH Đúng Giờ'].fillna(0).sum() + df['LH Trễ'].fillna(0).sum()
    sh_total_mtd = df['Shuttle Đúng Giờ'].fillna(0).sum() + df['Shuttle Trễ'].fillna(0).sum()
    lhot_mtd = (df['LH Đúng Giờ'].fillna(0).sum() / lh_total_mtd * 100) if lh_total_mtd > 0 else 0
    shot_mtd = (df['Shuttle Đúng Giờ'].fillna(0).sum() / sh_total_mtd * 100) if sh_total_mtd > 0 else 0
    ...
    # ==============================================================================
    # 4. QUẢN LÝ VẬN TẢI (CHỈ SỬA PHẦN NÀY)
    # ==============================================================================
    st.markdown(f"<h4 style='color: {primary_color};'>2. Quản lý Vận Tải (Hiển thị Số Chuyến Xe)</h4>", unsafe_allow_html=True)
    col4, col5 = st.columns([1, 1])

    with col4:
        # --- BIỂU ĐỒ LH: HIỂN THỊ TỔNG CHUYẾN XE (LOGIC CHỒNG) ---
        fig_lh = go.Figure()
        # Trace Đúng giờ (bên dưới), tắt nhãn
        fig_lh.add_trace(go.Bar(x=df['Ngày'], y=df['LH Đúng Giờ'], name="Đúng", marker_color='#10b981', 
                                text=None)) # Tắt nhãn inside
        # Trace Trễ (trên cùng), hiển thị nhãn Tổng ở ngoài đỉnh cột
        fig_lh.add_trace(go.Bar(x=df['Ngày'], y=df['LH Trễ'], name="Trễ", marker_color='#f43f5e', 
                                text=df['LH Tổng'], # Gắn nhãn là Tổng
                                textposition='outside', # Đặt nhãn ở ngoài đỉnh cột chồng
                                textfont=dict(weight='bold'))) 
        
        fig_lh.update_layout(title="Linehaul (LH) - Tổng số chuyến", barmode='stack', plot_bgcolor='white', 
                              margin=dict(t=40, b=10), legend=dict(orientation="h", y=-0.2))
        st.plotly_chart(fig_lh, use_container_width=True)

    with col5:
        # --- BIỂU ĐỒ SHUTTLE: HIỂN THỊ TỔNG CHUYẾN XE (LOGIC CHỒNG) ---
        fig_sh = go.Figure()
        # Trace Đúng giờ (bên dưới), tắt nhãn
        ...
        fig_sh.add_trace(go.Bar(x=df['Ngày'], y=df['Shuttle Trễ'], name="Trễ", marker_color='#f43f5e', 
                                text=df['Shuttle Tổng'], # Gắn nhãn là Tổng
                                textposition='outside', # Đặt nhãn ở ngoài đỉnh cột chồng
                                textfont=dict(weight='bold')))
        
        fig_sh.update_layout(title="Shuttle - Tổng số chuyến", barmode='stack', plot_bgcolor='white', 
                              margin=dict(t=40, b=10), legend=dict(orientation="h", y=-0.2))
        st.plotly_chart(fig_sh, use_container_width=True)

    # ==============================================================================
    # 5. CHI TIẾT SỐ CHUYẾN TRỄ & BACKLOG (THÊM BIỂU ĐỒ MỚI)
    # ==============================================================================
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"<h4 style='color: {primary_color};'>3. Chi tiết Số Chuyến Trễ & Hàng Tồn</h4>", unsafe_allow_html=True)
    col6, col7 = st.columns([2, 1])

    with col6:
        # --- BIỂU ĐỒ MỚI: SO SÁNH SỐ CHUYẾN TRỄ LH VS SHUTTLE ---
        fig_delay = go.Figure()
        # Cột trễ của LH
        fig_delay.add_trace(go.Bar(x=df['Ngày'], y=df['LH Trễ'], name="LH Trễ", marker_color='#ef4444', opacity=0.9,
                                   text=df['LH Trễ'], textposition='outside', textfont=dict(size=11)))
        # Cột trễ của Shuttle side-by-side
        fig_delay.add_trace(go.Bar(x=df['Ngày'], y=df['Shuttle Trễ'], name="Shuttle Trễ", marker_color='#f97316', opacity=0.9,
                                   text=df['Shuttle Trễ'], textposition='outside', textfont=dict(size=11)))
        
        # Barmode 'group' để đặt cột cạnh nhau
        fig_delay.update_layout(title="So sánh số chuyến TRỄ hàng ngày (LH vs Shuttle)", barmode='group', 
                                plot_bgcolor='white', margin=dict(t=40, b=10), legend=dict(orientation="h", y=1.1))
        st.plotly_chart(fig_delay, use_container_width=True)

    with col7:
        # --- BIỂU ĐỒ BACKLOG (DỊCH CHUYỂN TỪ MỤC TRÊN XUỐNG ĐÂY) ---
        ...
        fig_bl.update_layout(plot_bgcolor='white', margin=dict(t=40, b=10))
        st.plotly_chart(fig_bl, use_container_width=True)

    with st.expander("🔍 Chi tiết dữ liệu thô (GIỮ NGUYÊN)"):
        st.dataframe(df.set_index("Ngày").T, use_container_width=True)

# ==============================================================================
# 4. GỌI RENDER CHO TỪNG TAB (GIỮ NGUYÊN)
# ==============================================================================
with tab1:
