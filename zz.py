...
def render_dashboard(df, summary, primary_color):
    ...
    st.markdown(f"<h3 style='color: {primary_color}; font-weight: 800; margin-top: 30px; border-bottom: 2px solid {primary_color}; padding-bottom: 5px;'>1. Sản Lượng & Năng Suất</h3>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        # Bar Chart
        fig_prod_v.add_trace(go.Bar(x=df['Ngày'], y=df['Total Process Vol'], name="Sản lượng", marker_color='#38bdf8', opacity=0.85, 
        ...
        # Trend Line (Spline) - FIX TỪ GẠCH NGANG SANG ĐƯỜNG TĂNG GIẢM
        fig_prod_v.add_trace(go.Scatter(x=df['Ngày'], y=df['Total Process Vol'], name="Xu hướng",
        ...
        fig_prod_v.update_layout(yaxis=dict(range=[0, max_y_pv]), height=480, showlegend=False)
        st.plotly_chart(fig_prod_v, use_container_width=True)
        
    with col3:
        fig_prod_w = go.Figure()
        # Bar Chart
        fig_prod_w.add_trace(go.Bar(x=df['Ngày'], y=df['Total Process Wgt'], name="Trọng lượng", marker_color='#818cf8', opacity=0.85, 
                                    text=[f"<b>{format_vietnam(v)}</b>" if pd.notna(v) and v > 0 else "" for v in df['Total Process Wgt']], 
                                    textposition='outside', textfont=dict(size=14, color='#0f172a')))
        # Trend Line (Spline) - FIX TỪ GẠCH NGANG SANG ĐƯỜNG TĂNG GIẢM
        fig_prod_w.add_trace(go.Scatter(x=df['Ngày'], y=df['Total Process Wgt'], name="Xu hướng",
                                        line=dict(color='#ef4444', width=3, shape='spline'),
                                        mode='lines'))
        ...
        fig_prod_w.update_layout(yaxis=dict(range=[0, max_y_pw]), height=480, showlegend=False)
